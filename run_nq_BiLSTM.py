...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  # The second last layer embeddings are used as it preserved more info.
  embedding = model.get_sequence_output()
  max_seq_length = embedding.shape[1].value
  used = tf.sign(tf.abs(input_ids))
  lengths = tf.reduce_sum(used, reduction_indices=1)  # get the actual seq length
  ...
  bilstm_model = BiLSTM(embedded_chars=embedding, hidden_unit=lstm_size, cell_type=cell, num_layers=num_layers,
                        dropout_rate=dropout_rate, initializers=initializers, num_labels=2, seq_length=max_seq_length,
                        lengths=lengths, is_training=is_training)

  # The attention layer output is later used for answer type prediction.
  logits, attn = bilstm_model.add_bilstm_layer()
  ...
  '''
    Handling the learning of answer types, as this is joint bert model
    which means it will be used to handle both long and short answer questions.
    so, the ground-truth includes if it is answering the right type of question.
  '''
  attn_hidden_size = attn.shape[-1].value
  num_answer_types = 5  # YES, NO, UNKNOWN, SHORT, LONG
  answer_type_output_weights = tf.get_variable(
      "answer_type_output_weights", [num_answer_types, attn_hidden_size],
  ...
  answer_type_logits = tf.matmul(
      attn, answer_type_output_weights, transpose_b=True)
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    saved_vars = []
    for var in tvars:
      if not var.name.startswith(('Conv/', 'fully_connected/')):
