...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  final_hidden = tf.reshape(final_hidden,
                            [batch_size * seq_length, hidden_size, 1])
  WINDOW_SIZE = 32
  STRIDE = 1
  conv_ans = tf.contrib.layers.conv1d(final_hidden, 1, WINDOW_SIZE, 
                stride=STRIDE, padding='SAME')   
  conv_ans = tf.nn.relu(conv_ans) 
  tf.logging.info("ans span conv shape: %s" % conv_ans.shape)
  final_hidden = tf.squeeze(conv_ans, [2]) 
  final_hidden_shape = modeling.get_shape_list(final_hidden, expected_rank=2)
  convoluted_hidden_size = final_hidden_shape[1]

  output_weights = tf.get_variable(
      "cls/nq/output_weights", [2, convoluted_hidden_size],
  ...
  #tf.logging.info(answer_type_output_layer.shape)

  answer_type_output_layer = tf.reshape(answer_type_output_layer,
                                   [batch_size, answer_type_hidden_size, 1])

  WINDOW_SIZE = 32
  STRIDE = int(WINDOW_SIZE/2)
  conv = tf.contrib.layers.conv1d(answer_type_output_layer, 1, WINDOW_SIZE, 
                stride=STRIDE, padding='SAME')   
  conv = tf.nn.relu(conv) 
  tf.logging.info("question type conv shape: %s" % conv.shape)
  logits_ans_type = tf.squeeze(conv, [2]) 
  answer_type_logits = tf.contrib.layers.fully_connected(logits_ans_type, num_answer_types, 
                                    activation_fn=None)
